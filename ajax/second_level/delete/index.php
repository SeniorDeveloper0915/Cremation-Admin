...
if(!isset($_SESSION['admin']))
{
    ...
    die(json_encode($output));
      ...
      $query      = 'DELETE FROM `project` WHERE Second_Project_Id = ?';
      $parameters = array($projectId);
      $statement  = $db->prepare($query);
      $statement->execute($parameters);

      $query      = 'DELETE FROM `service` WHERE Second_Project_Id = ?';
      $parameters = array($projectId);
      $statement  = $db->prepare($query);
      $statement->execute($parameters);

      $query      = 'DELETE FROM `skill` WHERE Second_Project_Id = ?';
      $parameters = array($projectId);
      $statement  = $db->prepare($query);
      $statement->execute($parameters);

      $query      = 'DELETE FROM `second_project` WHERE id=?';
      $parameters = array($projectId);
      $statement  = $db->prepare($query);
      $statement->execute($parameters);
