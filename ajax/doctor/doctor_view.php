 ...
 */
$aColumns = array( 'id','Doctor_Name', 'Title_Id', 'Length', 'Certi_Number', 'Sort', 'Status', 'Release_Time');
$asColumns = array( 'id','Doctor_Name', 'Title_Id', 'Length', 'Certi_Number', 'Sort', 'Status', 'Release_Time');
$aaColumns = array( 'id','Doctor_Name', 'Title_Id', 'Length', 'Certi_Number', 'Sort', 'Status', 'Release_Time');
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    {
        if ($aaColumns[$i] == 'Title_Id')
        {
            $getDoctorTitle = $db->prepare("SELECT * FROM doctor_title where id = ?");
            $getDoctorTitle->execute(array($aRow[$aaColumns[$i]]));
            $rowTitle = $getDoctorTitle->fetch(PDO::FETCH_ASSOC);

            $row[] = $rowTitle['Doctor_Title'];
        }
        ...
        if($aaColumns[$i] == 'Status') {
            if($aRow[$aaColumns[$i]] == '1') {
                $row[] = '<a href="javascript:;" onclick="changeStatus('.$row[0].')" class="btn btn green btn-outline btn-sm">下架</a>&nbsp;
                          <a href="javascript:;" onclick="viewDoctor('.$row[0].')" class="btn btn-outline red btn-sm">查看</a>&nbsp
                          <a href="javascript:;" onclick="editDoctor('.$row[0].')" class="btn btn-outline blue btn-sm">修改</a>&nbsp
                          <a href="javascript:;" onclick="deleteDoctor('.$row[0].')" class="btn btn-outline black btn-sm">删除</a>';
            } else {
