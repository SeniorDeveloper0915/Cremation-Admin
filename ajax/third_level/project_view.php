...
$aColumns = array( 'id', 'First_Project_Id', 'Second_Project_Id', 'Project_Name', 'Sort', 'Status', 'Release_Time');
$asColumns = array( 'id', 'First_Project_Id', 'Second_Project_Id', 'Project_Name', 'Sort', 'Status', 'Release_Time');
$aaColumns = array( 'id', 'First_Project_Id', 'Second_Project_Id', 'Project_Name', 'Sort', 'Status', 'Release_Time');
...
/* DB table to use */
$sTable = "project";
$firstTable = "first_project";
$secondTable = "second_project";
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    {
        if ($aaColumns[$i] == 'id' || $aaColumns[$i] == 'Project_Name' || $aaColumns[$i] == 'Release_Time' || $aaColumns[$i] == 'Sort')
        {
            $row[] = $aRow[$aaColumns[$i]];
        }
        
        else if ( $aaColumns[$i] == 'First_Project_Id' )
        {
            $projectData = $db->prepare('SELECT * FROM first_project WHERE id=?');
            $projectData->execute(array($aRow[$aaColumns[$i]]));
            $rowProject = $projectData->fetch(PDO::FETCH_ASSOC);
            $row[] = $rowProject['Project_Name'];
        }

        else if ($aaColumns[$i] == 'Second_Project_Id' )
        {
        ...
        if($aaColumns[$i] == 'Status') {
            if($aRow[$aaColumns[$i]] == '1') {
                $row[] = '<a href="javascript:;" onclick="changeStatus('.$row[0].')" class="btn btn green btn-outline btn-sm">下架</a>&nbsp;
                          <a href="javascript:;" onclick="viewProject('.$row[0].')" class="btn btn-outline red btn-sm">查看</a>&nbsp
                          <a href="javascript:;" onclick="editProject('.$row[0].')" class="btn btn-outline blue btn-sm">修改</a>&nbsp
                          <a href="javascript:;" onclick="deleteProject('.$row[0].')" class="btn btn-outline black btn-sm">删除</a>';
            } else {
