 ...
 */
$aColumns = array( 'id','PhoneNumber', 'Question_Title', 'Question_Content', 'Question_Time', 'Answer_Time', 'Status', 'Reading_Volume');
$asColumns = array( 'id','PhoneNumber', 'Question_Title', 'Question_Content', 'Question_Time', 'Answer_Time', 'Status', 'Reading_Volume');
$aaColumns = array( 'id','PhoneNumber', 'Question_Title', 'Question_Content', 'Question_Time', 'Answer_Time', 'Status', 'Reading_Volume');
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    for ($i = 0; $i < count($aaColumns); $i ++)
    {
        if($aaColumns[$i] == 'Status') {
            if($aRow[$aaColumns[$i]] == '1') {
                $row[] = '<a href="javascript:;" onclick="answer('.$row[0].')" class="btn btn grey btn-outline btn-sm disabled">已回答</a>&nbsp;
                          <a href="javascript:;" onclick="view('.$row[0].')" class="btn btn-outline red btn-sm">查看</a>&nbsp
                          <a href="javascript:;" onclick="deleteQa('.$row[0].')" class="btn btn-outline black btn-sm">删除</a>';
            } else {
                $row[] = '<a href="javascript:;" onclick="answer('.$row[0].')" class="btn btn green btn-outline btn-sm">答复</a>&nbsp;
                          <a href="javascript:;" onclick="view('.$row[0].')" class="btn btn-outline red btn-sm">查看</a>&nbsp
                          <a href="javascript:;" onclick="deleteQa('.$row[0].')" class="btn btn-outline black btn-sm">删除</a>';
            }
        }
