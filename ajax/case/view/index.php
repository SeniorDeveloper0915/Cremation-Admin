 ...
 */
$aColumns = array( 'id', 'Hospital_Id', 'Title', 'Time', 'Sort', 'Status');
$asColumns = array( 'id', 'Hospital_Id', 'Title', 'Time', 'Sort', 'Status');
$aaColumns = array( 'id', 'Hospital_Id', 'Title', 'Time', 'Sort', 'Status');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "id";

/* DB table to use */
$sTable = "hospital_case";
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    {   
        if ($aaColumns[$i] != 'Status' && $aaColumns[$i] != 'Hospital_Id')
            $row[] = $aRow[ $aaColumns[$i] ];

        if($aaColumns[$i] == 'Hospital_Id') 
        {
            $getHospital = $db->prepare("SELECT * FROM hospital where id = ?");
            $getHospital->execute(array($aRow[$aaColumns[$i]]));
            $rowHospital = $getHospital->fetch(PDO::FETCH_ASSOC);
            $row[] = $rowHospital['Hospital_Name'];
        }
        ...
        if($aaColumns[$i] == 'Status') {
            if($aRow[$aaColumns[$i]] == '1') {
                $row[] = '<a href="javascript:;" onclick="changeStatus('.$row[0].')" class="btn btn green btn-outline btn-sm">下架</a>&nbsp;
                          <a href="javascript:;" onclick="viewCase('.$row[0].')" class="btn btn-outline red btn-sm">查看</a>&nbsp
                          <a href="javascript:;" onclick="editCase('.$row[0].')" class="btn btn-outline blue btn-sm">修改</a>&nbsp
                          <a href="javascript:;" onclick="deleteCase('.$row[0].')" class="btn btn-outline black btn-sm">删除</a>';
            } else {
